...
var color = d3.scale.ordinal().range([
    ...
    "#72C39B", "#80CEAA", "#80CCB3", "#7FC9BD", "#7FC7C6", "#7EC4CF", "#7FBBCF", "#7FB1CF", "#80A8CE", "#809ECE", "#8897CE", "#8F90CD", "#9788CD", "#9E81CC", "#AA81C5", "#B681BE", "#C280B7", "#CE80B0", "#D3779F", "#D76D8F", "#DC647E", "#E05A6D", "#E16167", "#E26962", "#E2705C", "#E37756", "#E38457", "#E39158", "#E29D58", "#E2AA59", "#E0B15B", "#DFB95C", "#DDC05E", "#DBC75F", "#E3CF6D", "#EAD67C", "#F2DE8A"]);  
...
d3.tsv(dataUrl, function(error, data) { 
  ...
  var categories = color.domain().map(function(name) { // Nest the data into an array of objects with new keys
    return {
      ...
      values: data.map(function(d) { // "values": which has an array of the dates and temperatures
        return {
          Year: d.Year, 
          temperature: parseFloat(d[name]),
          };
      }),
      visible: false //(name === "Glob" || name === "NHem" || name === "SHem")
    };
  });
  ...
  yScale.domain([
      findMinY(categories), findMaxY(categories)
  ]);
 ...
 var brush = d3.svg.brush()//for slider bar at the bottom
  ...
  issue.append("rect")
      ...
      .on("click", function(d){ // On click make d.visible 
        ...
        minY = findMinY(categories); // Find max Y temperature value categories data with "visible"; true
  ...
  function mousemove() { 
      ...
      hoverDate.text("Year " + format(graph_x)); // scale mouse position to xScale date and format it to show month and year
  ...
  function findMinY(data){  // Define function "findMaxY"
    var minYValues = data.map(function(d) { 
      if (d.visible){
        return d3.min(d.values, function(value) { // Return max temperature value
          return value.temperature; })
      }
    });
